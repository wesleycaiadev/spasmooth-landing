...
export default function KanbanPage() {
    ...
    return (
        <div className="min-h-screen relative animate-fadeIn">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-10 gap-4 w-full">
                <div>
                    <h1 className="text-2xl md:text-3xl font-serif font-bold text-slate-800 tracking-tight">Gestão de Leads</h1>
                    <p className="text-sm md:text-base text-slate-500 mt-1 md:mt-2 font-light">Organize e acompanhe seus atendimentos.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2 md:gap-3 w-full md:w-auto">
                    <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-inner flex-1 md:flex-none justify-center">
                        <button
                            onClick={() => setViewMode('kanban')}
                            className={`flex justify-center items-center gap-2 px-3 md:px-4 py-2 rounded-lg text-sm font-bold transition-all flex-1 md:flex-none ${viewMode === 'kanban' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            <KanbanSquare size={18} /> Kanban
                        </button>
                        <button
                            onClick={() => setViewMode('list')}
                            className={`flex justify-center items-center gap-2 px-3 md:px-4 py-2 rounded-lg text-sm font-bold transition-all flex-1 md:flex-none ${viewMode === 'list' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                    ...
                    <div className="flex gap-2 w-full md:w-auto">
                        <button
                            ...
                            className="flex-1 md:flex-none p-3 bg-slate-800 text-white shadow-sm rounded-xl hover:shadow-md hover:bg-slate-700 transition-all flex justify-center items-center gap-2 font-bold px-5 text-sm md:text-base"
                        >
                            <Plus size={20} /> Novo Lead
                        </button>
                        <button
                            onClick={fetchLeads}
                            className="p-3 bg-white/80 backdrop-blur-sm border border-white/50 shadow-sm rounded-xl hover:shadow-md hover:scale-105 transition-all text-cyan-700 shrink-0"
            ...
            {viewMode === 'kanban' ? (
                <div className="flex overflow-x-auto gap-4 md:gap-8 pb-12 custom-scrollbar snap-x snap-mandatory px-2 md:px-0">
                    {Object.entries(COLUMNS).map(([statusKey, config]) => (
                        <div
                            key={statusKey}
                            onDragOver={handleDragOver}
                            onDrop={(e) => handleDrop(e, statusKey)}
                            className={`w-[85vw] min-w-[320px] md:w-auto md:min-w-[350px] shrink-0 snap-center bg-white/60 backdrop-blur-md rounded-3xl p-4 md:p-5 h-auto border-2 ${draggedLeadId ? 'border-dashed border-cyan-300 transition-colors' : 'border-white/50 border-solid shadow-sm'} flex flex-col max-h-[calc(100vh-180px)]`}
                        >
                            {/* Column Header */}
                            <div className={`flex items-center justify-between mb-4 md:mb-6 p-4 rounded-2xl ${config.color} bg-opacity-80 backdrop-blur-sm shadow-sm`}>
                                <span className="font-bold tracking-wide text-sm uppercase">{config.label}</span>
                                <span className="bg-white/60 px-3 py-1 rounded-full text-xs font-extrabold shadow-sm text-slate-800">
