-- Only a pending or confirmed booking reserves time. Cancelled and completed
-- records remain as history, but must not prevent a new booking for the slot.
...
BEGIN
  ...
  WHILE v_slot+make_interval(mins=>v_duration)<=v_close LOOP
    IF v_slot >= now()+interval '30 minutes' AND NOT EXISTS (
      SELECT 1 FROM public.bookings
      WHERE professional_id=p_professional_id
        AND status IN ('pendente', 'confirmado')
        AND tstzrange(starts_at,ends_at,'[)') && tstzrange(v_slot,v_slot+make_interval(mins=>v_duration),'[)')
    ) THEN
...
CREATE OR REPLACE FUNCTION public.admin_delete_lead_and_booking(p_id uuid) RETURNS void
LANGUAGE plpgsql SECURITY INVOKER SET search_path=public,pg_temp AS $$
BEGIN
  -- Keep the booking as a cancelled audit record, but it no longer blocks a slot.
  UPDATE public.bookings
    SET status='cancelado',
        cancelled_at=coalesce(cancelled_at, now()),
        cancelled_by=coalesce(cancelled_by, 'painel_admin_exclusao')
    WHERE id=p_id AND status IN ('pendente', 'confirmado');

  DELETE FROM public.leads WHERE id=p_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'NOT_FOUND'; END IF;
END $$;
REVOKE ALL ON FUNCTION public.admin_delete_lead_and_booking(uuid) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.admin_delete_lead_and_booking(uuid) TO service_role;
